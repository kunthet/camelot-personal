...
from .utils import (
    TemporaryDirectory,
    get_page_layout, get_all_page_layout,
    get_text_objects,
    get_rotation,
    is_url,
    download_url,
...
class PDFHandler(object):
    ...
    def _get_layout_all_pages(self, filepath):

        p_layout_kwargs = []
        layouts, dims = get_all_page_layout(filepath)
        for layout, dim in zip(layouts, dims):
            # fix rotated PDF
            ...
            p_layout_kwargs.append(layout)
            
        return p_layout_kwargs

    def _save_page_all_pages(self, filepath, temp):
        with open(filepath, "rb") as fileobj:
            ...
            layout, dim = None, None
            p_layout_kwargs = []
            i = 0
            for page in infile.pages:
                i+=1
                fpath = os.path.join(temp, f"page-{i}.pdf")
                ...
                with open(fpath, "wb") as f:
                    outfile.write(f)
                layout, dim = get_page_layout(fpath) #if not layout else layout, dim
    ...
    def parse(
        ...
        with TemporaryDirectory() as tempdir:
            if self._all_pages:
                p_layout_kwargs = self._get_layout_all_pages(self.filepath)
            else:
                p_layout_kwargs = []
                for p in self.pages:
